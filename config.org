      ...
      (use-package elpa-mirror
        ...
        (defun fn/update-local-elpa-mirror ()
          ...
          (when (file-newer-than-file-p package-user-dir fn/local-elpa-mirror-dir)
            (message "Updating local elpa mirror... in case of emergency.")
            (elpamr-create-mirror-for-installed)))

        (add-hook 'kill-emacs-hook #'fn/update-local-elpa-mirror))
    #+END_SRC
...
*** org-capture
    ...
    #+BEGIN_SRC emacs-lisp :block-id 42c9d746-9fe2-506f-2f27-a44df1f38666
      (use-feature org-capture
        ...
        (defun fn/org-capture-place-template (&optional inhibit-wconf-store)
          "This is hacked form of `org-capture-place-template' to make it open in the same window."
        ...
        ;; Overrides here
        (defun fn/override-org-capture-display-functions (&rest args)
          "Override org-capture functions to make it display in the same window"
          (fset 'org-capture-place-template #'fn/org-capture-place-template)
          (fset 'org-switch-to-buffer-other-window #'fn/org-switch-to-buffer-other-window))

        (advice-add 'org-capture :before 'fn/override-org-capture-display-functions))
    #+END_SRC
...
*** IDE
    ...
    #+BEGIN_SRC emacs-lisp :block-id eae7c8e8-8138-a1f4-5ebf-15bd6953e699
      (use-package jdee
        ...
        :disabled t
        :config
        (defconst fn/jdee-server-dir (expand-file-name "jdee-server/" fn/custom-module-dir)
          "My jdee server dir.")

        (setq jdee-server-dir (expand-file-name "./" fn/jdee-server-dir)))
   ...
   #+BEGIN_SRC emacs-lisp :block-id 7cf5dadb-eb6b-a004-1566-eb12464e2e44
     (use-package erc
       ...
       (setq erc-try-new-nick-p t
          ...
          erc-autojoin-channels-alist (list (list "freenode.net"
                                            "#emacs"
                                            "#conkeror"
                                            "#flowtype"
                                            "#rethinkdb"))
