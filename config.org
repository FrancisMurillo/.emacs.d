...
** Personal Information
   ...
   #+BEGIN_SRC emacs-lisp
     (defconst personal-file-name "personal.el"
       "Personal configuration file name")

     (defconst fn/personal-file (expand-file-name personal-file-name user-emacs-directory)
...
** Coding
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      ;; Make M-a go to period instead of the beginning of the paragraph
          ...
          (when (projectile-project-p)
            (let* ((current-project-root (projectile-project-root))
              ...
              (when (file-exists-p project-init-file)
                (message "Loading project init file for %s" current-project-root)
                (unwind-protect
          ...
          (when (projectile-project-p)
            (let* ((current-project-root (projectile-project-root))
              ...
              (when (file-exists-p project-local-init-file)
                (message "Loading project local file for %s " current-project-root)
                (unwind-protect
...
** org
    ...
    #+BEGIN_SRC emacs-lisp
      (use-package org
        ...
        (defconst fn/org-event-file-name "event.org"
          "My event planning file name")

        (defconst fn/org-event-file (expand-file-name fn/org-event-file-name org-directory)
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defun fmw/restore-buffers (window-movement)
       ...
       (lexical-let ((current-buffers (mapcar 'window-buffer (window-list)))
         ...
         (mapc (lambda (pair)
                 (lexical-let* ((old-buffer (car pair))
                                (current-window (cdr pair))
                                (the-buffer (if old-buffer old-buffer empty-buffer)))
                   (when current-window
                     (set-window-buffer current-window the-buffer))))
               (-zip-fill nil current-buffers (window-list)))))
     ...
     (defun fmc/buy-coin ()
       ...
       (lexical-let* ((amount (read-number "Enter amount: "))
         ...
         (when confirm-action
           ...
           (message "Successfully bought %f at %f" amount current-buy))))
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defun fmd/get-title (&optional selected-buffer)
       ...
       (with-current-buffer (if selected-buffer selected-buffer (buffer-name))
         (lexical-let* ((title-regex "\#\\+TITLE:\s+\\(.*\\)")
                        (title-match (s-match title-regex (buffer-substring-no-properties (point-min) 1000)))
                        (title (if title-match (cadr title-match)
                                 nil)))
           title)))
...
* My Bindings
  ...
  #+BEGIN_SRC emacs-lisp
    (defun fn/find-current-key (key)
      "Find the current key active"
      (reduce (lambda (x y)
                (or x y))
              (mapcar (lambda (map)
                        (lookup-key map key))
    ...
    (defun fn/trigger-key (key)
      ...
      (lexical-let* ((key-trigger key)
                     (trigger (lambda ()
                                (interactive)
                                (let ((key-command (fn/find-current-key key-trigger)))
                                  (cond
                                   (key-command
    ...
    ;; Motion
    (define-key global-map (kbd "C-!") (fn/trigger-key (kbd "C-<return>")))
    (define-key global-map (kbd "M-!") (fn/trigger-key (kbd "M-<return>")))

    ;; Keys
    (define-key global-map (kbd "M-n") (fn/trigger-key (kbd "DEL")))
    ...
    ;; Windows
    (define-key global-map (kbd "C-c m w h") 'fmw/four-part-horizontal-window-layout)
    (define-key global-map (kbd "C-c m w v") 'fmw/four-part-vertical-window-layout)
    (define-key global-map (kbd "C-c m w t") 'fmw/three-part-left-window-layout)
    (define-key global-map (kbd "C-c m w 3") 'fmw/three-part-vertical-window-layout)
    ...
    ;; Commands
    (define-key global-map (kbd "C-x C-x") (fn/trigger-key (kbd "M-x")))
