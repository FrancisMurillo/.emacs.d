    ...
    #+BEGIN_SRC emacs-lisp :block-id d9ff5d65-9c4a-24ee-d9e8-81459f7851d3
      ...
      (use-package elpa-mirror
        ...
        (defun fn/batch-update-local-elpa-mirror ()
          ...
          (interactive)
          (lexical-let ((emacs-command "emacs")
              (local-elpa-mirror-glob (expand-file-name "*" fn/local-elpa-mirror-dir)))
            (deferred:$
              (deferred:next
                (lambda () (message "Starting to update... Cleaning up old packages.")))
              (deferred:process-shell-bufferc it ;; TODO: Windows?
                "rm" "-rf"
                local-elpa-mirror-glob)
              (deferred:nextc it
        ...
        (add-hook 'after-init-hook 'fn/update-local-elpa-mirror)
