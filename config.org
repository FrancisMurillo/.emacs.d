...
**** helm-smex
     ...
     #+BEGIN_SRC emacs-lisp :block-id 7f1cc404-a888-5187-4df6-dcba60ae2373
       ...
       (use-package helm-smex
         ...
         :bind (("M-x" . helm-smex))
         :config
         (defun fn/command-keybinding-to-string (cmd)
           "Copied over `org-drill' to avoid dependencies."
           (let ((key (where-is-internal cmd overriding-local-map t)))
             (if key (key-description key))))

         (defun fn/helm-smex-transformer (candidates)
           "Add keybinding to transformer with CANDIDATES."
           (mapcar
            (lambda (candidate)
              (cons
               (let ((binding (fn/command-keybinding-to-string (intern-soft candidate))))
                 (if (null binding)
                     candidate
                   (format "%s (%s)"
                           candidate
         ...
         ;; Override helm-smex source to have a candidate-transformer which just adds key binding
         (defclass helm-smex-source (helm-source-sync)
           ((init :initform 'helm-smex--init)
            (candidates :initform 'smex-ido-cache)

            (candidate-transformer :initform #'fn/helm-smex-transformer)
        ...
        (use-package ob-haskell
          ...
          (defun fn/cleanup-org-haskell-buffer (orig-fun &rest args)
            ...
            (lexical-let ((org-haskell-file-name (buffer-file-name)))
              (prog1
                  (apply orig-fun args)
                (when (and (file-exists-p org-haskell-file-name)
                         (fn/org-haskell-buffer-p))
...
** Fluxus                                                      :experimental:
   Nice graphical environment

   #+BEGIN_SRC emacs-lisp :block-id fc504796-e991-d5e2-1620-4df729d08197
     (use-package fluxus-mode
       :ensure t
       :demand t)
   #+END_SRC
