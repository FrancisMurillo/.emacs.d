      ...
      (unless noninteractive
        ...
        (use-package winum
          ...
          (setq window-numbering-scope 'global
             winum-auto-setup-mode-line nil ;; moder.el does this for me
             winum-keymap-prefix (kbd "C-c n w"))
   ...
   #+BEGIN_SRC emacs-lisp :block-id 1ecfd02f-87eb-6c8a-0e20-68780b2e1e7b
     (use-package camcorder
       ...
       (defun fn/camcorder-init ()
         "Load an initial script when recording."
       ...
       (add-hook 'camcorder-mode-hook #'fn/camcorder-demo-buffer)
       (add-hook 'camcorder-mode-hook #'fn/camcorder-init t)
...
** w3m
   ...
   #+BEGIN_SRC emacs-lisp :block-id 4ef8aaa9-5b2f-30e3-c126-323935e0f9f6
     (use-package w3m
       ...
       (setq w3m-bookmark-file (expand-file-name "w3m-bookmark.html" fn/cache-dir)
       ...
       ;; Video Player
       (require 'w3m-cookie)
       (defconst fn/w3m-video-executable "vlc"
         "The executable that can run a network video stream.")

       (defconst fn/w3m-video-args (list)
       ...
       (defun fn/w3m-video-url-p (url)
         ...
         (ignore-errors
           (lexical-let* ((pieces (w3m-parse-http-url url))
               (host (elt pieces 1)))
             (if (or (string= host "www.youtube.com"))
                 url nil))))
       ...
       (defun fn/w3m-view-this-video-or-url (&rest args)
         "View this video or url."
         (lexical-let ((video-url (fn/w3m-video)))
           (when
               (and video-url
                  (yes-or-no-p
                   (format "This url is a video, view it with %s?" fn/w3m-video-executable)))
             (fn/w3m-view-video))))

       (advice-add 'w3m-view-this-url :before #'fn/w3m-view-this-video-or-url)
     ...
     Thanks to https://www.emacswiki.org/emacs/WThreeMTabs"
       ...
       (use-feature w3m-mode--mode-icon
         (all-the-icons)
         (fn/add-major-mode-icon
          'w3m-mode
