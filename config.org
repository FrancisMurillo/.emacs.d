   ...
   #+BEGIN_SRC emacs-lisp :block-id 4ef8aaa9-5b2f-30e3-c126-323935e0f9f6
     (fn/use-executables w3m-check
       ...
       (use-package w3m
         ...
         (defcustom fn/w3m-ignore-arrived-url-functions ()
         ...
         (fn/use-executables w3m-polipo-tor
           (tor polipo w3m)
           (defcustom fn/w3m-use-polipo-proxy 'nothing
             "Check if a polipo proxy should be used.")

           (when (eq fn/w3m-use-polipo-proxy
                     (default-value 'fn/w3m-use-polipo-proxy))
             (setq fn/w3m-use-polipo-proxy (yes-or-no-p "Use polipo in browsing? "))

             (with-eval-after-load 'savehist
               (add-to-list 'savehist-additional-variables 'fn/w3m-use-polipo-proxy)))

           (when fn/w3m-use-polipo-proxy
             (defconst fn/w3m-polipo-cache-dir (expand-file-name "polipo-cache" fn/cache-dir)
               "Polipo cache directory.")

             (make-directory fn/w3m-polipo-cache-dir t)

             (defcustom fn/w3m-tor-port 9050
               "Tor port")

             (defcustom fn/w3m-polipo-port 18123
               "Polipo port")

             (defvar fn/w3m-polipo-conf-file (expand-file-name "polipo-w3m-conf" fn/cache-dir)
               "Polipo configuration.")

             (defun fn/w3m-polipo-update-conf ()
               "Create/update `fn/w3m-polipo-conf-file' with new configurations."
               ...
               (setq w3m-command-arguments
                  (cl-remove-if (lambda (value)
                                  (or
                                   (string-prefix-p "http_proxy=https://127.0.0.1:" value)
                                   (string= "-o" value)))
                                w3m-command-arguments))


               (add-to-list 'w3m-command-arguments
                            "-o" t)

               (add-to-list 'w3m-command-arguments
                            (format
                             "http_proxy=https://127.0.0.1:%s/"
                             (number-to-string fn/w3m-polipo-port))
                            t))

             (fn/w3m-polipo-update-conf)
             ...
             (defun fn/w3m-polipo-start-process (&rest args)
               "Start `fn/w3m-polipo-process'."
               (fn/w3m-polipo-kill-process)

               (message "Starting polipo process.")
               (setq fn/w3m-polipo-process
                  (start-process "w3m-polipo-tor" "*w3m-polipo-tor*" "polipo" "-c" fn/w3m-polipo-conf-file)))

             (defun fn/w3m-polipo-kill-process (&rest args)
             ...
             (add-hook 'w3m-mode-hook #'fn/w3m-polipo-start-process)
             (advice-add 'w3m-quit :after #'fn/w3m-polipo-kill-process)
     ...
     Primary check comes from `w3m-parse-http-url' by direct string match."
         ...
         (add-hook 'w3m-fontify-after-hook #'fn/w3m-fontify-summary)
