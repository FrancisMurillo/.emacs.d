...
** Rust
   ...
   #+begin_src emacs-lisp :block-id 3461f079-e772-71d2-bdb2-ae1e84009f88
     ...
     (use-package cargo
       ...
       :config
       (defun fn/cargo-test-add-nocapture-flag (orig-fun &rest args)
         "When running tests, add `--nocapture' flag to allow printing out variables."
         (let ((cargo-process--command-flags
                (append cargo-process--command-flags
                        "-- --nocapture")))
           (apply orig-fun args)))

       (advice-add 'cargo-process-current-test :around #'fn/cargo-test-add-nocapture-flag)
       (advice-add 'cargo-process-test :around #'fn/cargo-test-add-nocapture-flag))
