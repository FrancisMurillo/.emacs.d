       ...
       `(use-feature helm-projectile
                    ...
                    (use-package helm-projectile
                      ...
                      (defun fn/custom-helm-completion (prompt choices)
                        ...
                        (lexical-let*
                            ((fn-notation (lambda (path)
                                            (lexical-let ((fn-separator " |> ")
                                                          (fn-pieces (f-split path)))
                                              (string-join (reverse fn-pieces) fn-separator))))
                             (relative-parent-path (lambda (path relative-path)
                                                     (lexical-let
                                                       ...
                                                       (string-join
                                                        ...
                                                        (f-path-separator)))))
                             (map-head (lambda (f pair)
                                         (list (funcall f (car pair)) (cdr pair))))
                             (swap-pair (lambda (pair)
                                          (cons (cdr pair) (car pair))))
                             (pair-as-label (lambda (pairs)
                                              (lexical-let* ((labels (mapcar #'cdr pairs))
                                                             (label-lengths (mapcar #'length labels))

                                                             (max-label-length (apply #'max label-lengths))

                                                             (label-format
                                                              (concat
                                                               "%-"
                                                               (number-to-string (+ 5 max-label-length))
                                                               "s"))
                                                ...
                                                (lambda (pair)
                                                  (lexical-let*
                                                    ...
                                                    (format display-format display-name display-description))))))
                             ...
                             (mapped-choices
                              (mapcar (-compose
                                       ...
                                       swap-pair)
                                      refined-choices)))
                          (car(helm-comp-read prompt mapped-choices
                                              :must-match 'confirm))))
