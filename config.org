...
** w3m
   ...
   #+BEGIN_SRC emacs-lisp :block-id 4ef8aaa9-5b2f-30e3-c126-323935e0f9f6
     (use-package w3m
       ...
       (defun fn/w3m-kill-current-url ()
         "Kill current url of page."
         (interactive)
         (if (null w3m-current-url)
             (error "Buffer has no `w3m-current-url'")
           (kill-new w3m-current-url)
           (message "Killed %s" w3m-current-url)))

       (define-key w3m-mode-map (kbd "C-c b u") #'fn/w3m-kill-current-url))
   #+END_SRC
