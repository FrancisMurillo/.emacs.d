     ...
     (unless noninteractive
       (use-package mu4e
         ...
         (progn ;; Mail count
           ...
           (defun fn/mu4e-update-unread-message-count (mail-count)
             "Update `fn/mu4e-update-unread-message-count'."
             (setq fn/mu4e-current-unread-message-count mail-count))

           (add-hook 'mu4e-view-mode-hook #'fn/mu4e-current-unread-message-count)
           (add-hook 'mu4e-index-updated-hook #'fn/mu4e-current-unread-message-count)
           (add-hook 'mu4e-msg-changed-hook #'fn/mu4e-current-unread-message-count))
   ...
   #+BEGIN_SRC emacs-lisp :block-id 2f91f506-c8a6-85b4-769d-3fb460019887
     ...
     (use-package slack
       ...
       (progn
         ...
         (fn/slack-update-unread-message-count)
         (advice-add 'slack-ws-handle-message :after #e'fn/slack-update-unread-message-count))
