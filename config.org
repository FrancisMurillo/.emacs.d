...
** Startup
   ...
   #+BEGIN_SRC emacs-lisp :block-id e3857d2d-0e50-e886-7186-cd58a7fd80ee
     ...
     (defvar fn/my-lightning "My Lightning... "
       "A reference to continue?987654321.
     May I accept the lightning.")

     (defvar fn/my-prayer "My Lightning... My Prayer..."
   ...
   #+BEGIN_SRC emacs-lisp :block-id e8344e55-09ff-27a8-a6c0-8d76e44cd3f0
     (use-package log4e
       ...
       (defvar fn/chat-server-name-face 'fn/chat-server-name-face
         "Slack team name face var")

       (defvar fn/chat-channel-name-face 'fn/chat-channel-name-face
         "Slack room name face var")

       (defvar fn/chat-chatter-name-face 'fn/chat-chatter-name-face
         "Slack sender name face var")

       (defvar fn/chat-message-face 'fn/chat-message-face
         "Slack message face var")

       (defun fn/chat-log (server channel chatter message)
         "Log SERVER, CHANNEL, CHATTER and MESSAGE."
         (let* ((out (format "[%s/%s] %s: %s"
                          (propertize server 'face 'fn/chat-server-name-face)
                          (propertize channel 'face 'fn/chat-channel-name-face)
                          (propertize chatter 'face 'fn/chat-chatter-name-face)
                          (propertize message 'face 'fn/chat-message-face))))
           (chat--log-info out)
           out))
       ...
       (defun fn/chat-log-flush ()
         "Save log for posterity."
         (save-window-excursion
           (fn/chat-log-open)
           (let ((new-log (buffer-substring-no-properties (point-min) (point-max)))
       ...
       (use-feature log4e-icon
         (all-the-icons)
         (add-to-list
          'all-the-icons-mode-icon-alist
          (list 'log4e-mode 'all-the-icons-octicon "clippy" :v-adjust 0.0))))
   #+END_SRC
...
** all-the-icons
   ...
   #+BEGIN_SRC emacs-lisp :block-id 1c07422d-833b-c29a-44dd-3c62d3498e7f
     (use-package all-the-icons
       ...
       (add-to-list
        'all-the-icons-mode-icon-alist
        (list 'special-mode 'all-the-icons-faicon "birthday-cake" :v-adjust -0.1))

       (add-to-list
        'all-the-icons-mode-icon-alist
        (list 'fundamental-mode 'all-the-icons-faicon "pencil" :v-adjust -0.1))

       (add-to-list
        'all-the-icons-mode-icon-alist
        (list 'compilation-mode 'all-the-icons-faicon "cog" :v-adjust -0.1))

       (add-to-list
        'all-the-icons-mode-icon-alist
        (list 'process-menu-mode 'all-the-icons-faicon "list" :v-adjust -0.1)))
   #+END_SRC
...
** shell
   ...
   #+BEGIN_SRC emacs-lisp :block-id da6d7eb9-45f4-ee00-154d-11e39b59d7bf
     ...
     (use-feature shell-icon
       (all-the-icons)
       (add-to-list
        'all-the-icons-mode-icon-alist
        (list 'shell-mode 'all-the-icons-faicon "terminal" :v-adjust 0.0)))
   #+END_SRC
...
** prodigy
   ...
   #+BEGIN_SRC emacs-lisp :block-id dc93746e-c346-c682-730d-a3d26b5e258a
     ...
     (use-package prodigy
       ...
       (use-feature prodigy-icons
         (all-the-icons)
         (add-to-list
          'all-the-icons-mode-icon-alist
          (list 'prodigy-mode 'all-the-icons-faicon "cogs" :v-adjust -0.1))
         (add-to-list
          'all-the-icons-mode-icon-alist
          (list 'prodigy-view-mode 'all-the-icons-faicon "cog" :v-adjust -0.1))))
   #+END_SRC
** emamux
     ...
     (use-package alert
       ...
       (defun fn/alert-log-open ()
         ...
         (if (fboundp 'alert--log-open-log)
             (alert--log-open-log-if-debug)
           (error "No alert log yet")))
...
** ledger
   ...
   #+BEGIN_SRC emacs-lisp :block-id 4e3b93da-bda6-0881-7e81-3e23e833f1cc
     (use-package ledger-mode
       ...
       (use-feature ledger-icon
         (all-the-icons)
         (add-to-list
          'all-the-icons-mode-icon-alist
          (list 'ledger-mode 'all-the-icons-faicon "money" :v-adjust -0.1))
         (add-to-list
          'all-the-icons-mode-icon-alist
          (list 'ledger-report-mode 'all-the-icons-faicon "line-chart" :v-adjust -0.1))))
   #+END_SRC
...
*** Project
    ...
    #+BEGIN_SRC emacs-lisp :block-id 08cd7439-df60-4b11-e8a1-17917ed8dd26
      ...
      (use-feature cask-icon
        (all-the-icons)
        (add-to-list
         'all-the-icons-mode-icon-alist
         (list 'cask-mode 'all-the-icons-faicon "glass" :v-adjust 0.1)))
    ...
    #+BEGIN_SRC emacs-lisp :block-id f4f7c05b-8948-6331-8550-71e06225c233
      (use-package js2-mode
        ...
        (use-feature js-icon
          ...
          (add-to-list
           'all-the-icons-mode-icon-alist
           (list 'js2-jsx-mode 'all-the-icons-alltheicon "javascript" :v-adjust 0.1))))
     ...
     (use-package erc
       ...
       (use-feature erc-message-alert
         ...
         (defun fn/erc-message-alert (message)
           ...
           (let ((raw-message (substring-no-properties message)))
       ...
       (use-feature erc-icon
         (all-the-icons)
         (add-to-list
          'all-the-icons-mode-icon-alist
          (list 'erc-mode 'all-the-icons-faicon "exchange" :v-adjust 0.0))))
   #+END_SRC
     ...
     (use-package jabber
       ...
       (use-feature jabber-alert-custom
         ...
         (defun fn/jabber-message-notify (from buffer text title)
           "Just a small unobtrusive notifcation"
           (alert (fn/chat-log "google" user-mail-address from text)
                  :category 'jabber
                  :style 'fringe
       ...
       (use-feature jabber-chat-icons
         (all-the-icons)
         (add-to-list
          'all-the-icons-mode-icon-alist
          (list 'jabber-chat-mode 'all-the-icons-faicon "comments" :v-adjust -0.1))
         (add-to-list
          'all-the-icons-mode-icon-alist
          (list 'jabber-roster-mode 'all-the-icons-faicon "building-o" :v-adjust -0.1)))
...
** slack
   ...
   #+BEGIN_SRC emacs-lisp :block-id 2f91f506-c8a6-85b4-769d-3fb460019887
     ...
     What I do is use the fringe to notify me instead of `libnotify'."
       ...
       (use-feature slack-icon
         (all-the-icons)
         (add-to-list
          'all-the-icons-mode-icon-alist
          (list 'slack-mode 'all-the-icons-faicon "commenting-o" :v-adjust 0.0))))
   #+END_SRC
