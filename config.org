...
** prodigy
   ...
   #+BEGIN_SRC emacs-lisp :block-id dc93746e-c346-c682-730d-a3d26b5e258a
     ...
     Also, check and persist `:on-stop' and `:on-start' property"
     ...
     `:on-start' which runs after it."
       ...
       ;; Prodigy recipes
       (fn/use-executables prodigy-tags-check
         (jekyll)
         (prodigy-define-tag
           :name 'jekyll
           :command "jekyll"
   ...
   #+BEGIN_SRC emacs-lisp :block-id 4ef8aaa9-5b2f-30e3-c126-323935e0f9f6
     (fn/use-executables w3m-check
       ...
       (use-package w3m
         ...
         ;; Privoxy-Polipo-Tor Secure Browsing
         (fn/use-executables w3m-privoxy-polipo-tor
           ...
           (defun fn/w3m-service-start (&rest args)
             ...
             (when (and (prodigy-service-started-p fn/w3m-polipo-service)
           ...
           (defun fn/w3m-service-init (&rest args)
             ...
             (unless (and (prodigy-service-started-p fn/w3m-polipo-service)
                        (prodigy-service-started-p fn/w3m-tor-service)
                        (prodigy-service-started-p fn/w3m-privoxy-service))
           ...
           (defun fn/w3m-privoxy-polipo-tor-start-process (&rest args)
             "Start services"
             (interactive)
           ...
           (defun fn/w3m-privoxy-polipo-tor-kill-process (&rest args)
             "Stop special services."
             (interactive)
             (prodigy-stop-service fn/w3m-polipo-service)
             (prodigy-stop-service fn/w3m-tor-service)
             (prodigy-stop-service fn/w3m-privoxy-service))

           (add-hook 'kill-emacs-hook #'fn/w3m-privoxy-polipo-tor-kill-process)
           ...
           (when (yes-or-no-p "Start privoxy, polipo and tor for w3m? ")
             (fn/w3m-privoxy-polipo-tor-start-process)))
