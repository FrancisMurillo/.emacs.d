...
** w3m
   ...
   #+BEGIN_SRC emacs-lisp :block-id 4ef8aaa9-5b2f-30e3-c126-323935e0f9f6
     (use-package w3m
       ...
       (defconst fn/w3m-sumy-executable "sumy"
         "The executable that can run a summarize articles.")

       (defconst fn/w3m-sumy-args
         (list
          "lex-rank"
          "--length" "5"
          "--url"
       ...
       (defun fn/w3m-summarize-url (url)
         ...
         (deferred:nextc
           ...
           (lambda (summary-output)
             (split-string (string-trim-right summary-output) "\n"))))

       (defun fn/w3m-fontify-summary ()
         "Fontify summary points."
         (lexical-let ((fontifier
              (lambda (text)
                (message "Pointing out %s" text)
                (let ((inhibit-read-only t)
                    (new-text (string-join (split-string text " ") "\n*\s*")))
                  (goto-char (point-min))
