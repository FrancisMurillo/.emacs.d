...
*** Tree View
    ...
    #+BEGIN_SRC emacs-lisp :block-id d1c4a74d-3098-f6ef-2636-3b6af0d39cbc
      (use-package speedbar
        :defer t
        :config ;; I prefer the same frame version
...
*** Linter
    ...
    #+BEGIN_SRC emacs-lisp :block-id da802e8e-8302-f301-51f5-6042384a98a7
      (unless noninteractive
        ...
        (fn/use-executables eslint-project-relative
          (eslint-project-relative)
          (fn/use-feature flycheck-eslint
            (flycheck js2-mode)
...
*** Git Gutter
    ...
    #+BEGIN_SRC emacs-lisp :block-id 7e3ab6c8-050f-9112-3b81-95fa8f85452f
      (defconst fn/git-gutter-map (fn/make-prefixed-keymap (kbd "C-c M-g"))
        "My custom git-gutter map.")
      ...
      (use-package git-gutter
        ...
        :commands (git-gutter)
        :bind (:map fn/git-gutter-map
                    ("g" . git-gutter:statistic)
   ...
   #+BEGIN_SRC emacs-lisp :block-id 2f91f506-c8a6-85b4-769d-3fb460019887
     ...
     (use-package slack
       ...
       (progn
         ...
         (defun fn/slack-current-buffer-channel-p ()
           "Check if the current buffer is a channel."
           (with-current-buffer (current-buffer)
             (and (boundp 'slack-current-room-id)
                  (boundp 'slack-current-team-id))))

         (defun fn/slack-goto-next-unread-channel ()
           "Go to next unread channel."
           (interactive)
           (unless (fn/slack-current-buffer-channel-p)
             (error "Not in a slack buffer. Invoke this command in a slack buffer."))
           (let ((team (slack-team-select))
                 (channel-id slack-current-room-id))
             (with-slots (channels) team
               (let* ((partitioned-channels
                       (-partition-by
                        (lambda (channel)
                          (not (string= (oref channel id) channel-id)))
                        channels))
                      (sequenced-channels
                       (if (<= (length partitioned-channels) 1)
                           nil
                         (let ((current-channels partitioned-channels))
                           (while (not (= (length (car current-channels)) 1))
                             (setq current-channels (-rotate -1 current-channels)))
                           (apply #'-concat (cdr current-channels)))))
                      (unread-channel
                       (-first
                        (lambda (channel)
                          (not (zerop (oref channel unread-count-display))))
                        sequenced-channels)))
                 (if (null unread-channel)
                     (error "No unread channel found.")
                   (slack-room-create-buffer unread-channel team)))))))
