...
** Startup
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defconst fn/gc-log (expand-file-name ".gc-log" fn/cache-dir)
       "A log of gcing")

     (defun fn/log-gc (orig-gc &rest args)
       "Log gc event duration with ORIG-GC and its ARGS."
       (let ((start-time (current-time))
         ...
         (prog1
           ...
           (append-to-file
            (format "%s :: %s\n"
                    (format-time-string "%F-%R-%N" start-time)
                    duration)
            nil
            fn/gc-log))))

     (advice-add 'garbage-collect :around #'fn/log-gc)
      ...
      (use-package paradox
        ...
        (use-feature icon-paradox
          ...
          (add-to-list
           ...
           (list 'paradox-menu-mode 'all-the-icons-octicon "package"))))
    #+END_SRC
