...
*** elpa-mirror
    ...
    #+BEGIN_SRC emacs-lisp :block-id d9ff5d65-9c4a-24ee-d9e8-81459f7851d3
      (when (and (boundp 'fn/user-github-name)
               (file-exists-p fn/external-module-dir))
   ...
   #+BEGIN_SRC emacs-lisp :block-id 4f5f3d2b-61ae-d33b-8316-4c20b3e9c8c0
     (use-package org
       ...
       :bind (("C-c l" . org-store-link)
              ...
              ("C-c c" . org-capture))
       :config
       (when (file-exists-p fn/external-module-dir)
         (defconst fn/org-dir (expand-file-name "nobody-library" fn/external-module-dir)
         ...
         (setq org-directory (concat fn/org-dir "/")))
...
** ledger
   ...
   #+BEGIN_SRC emacs-lisp :block-id 4e3b93da-bda6-0881-7e81-3e23e833f1cc
     (defconst fn/ledger-file (expand-file-name "ledger.dat.gpg"
                                         (expand-file-name "nobody-library"
                                                           fn/external-module-dir))
       "My ledger file.")
     ...
     (unless noninteractive
       (when (boundp 'fn/external-module-dir)
...
** Mail
   ...
   #+BEGIN_SRC emacs-lisp :block-id 188b9d05-dd2e-1564-0453-b638bd6e103e
     (defun fn/imap-service-setup ()
       "Setup an IMAP `prodigy' service."
       (interactive)
       (when (or (executable-find "offlineimap")
                (executable-find "mbsync"))
         (require 'prodigy)

         (defconst fn/imap-service-name "imap-offlineimap"
           "The IMAP service name.
     The name might be taken from either `offlineimap' or `mbsync'")

         (prodigy-define-tag
           :name 'imap)

         (fn/prodigy-define-service
          :name fn/imap-service-name
          :tags `(imap ,(if (executable-find "offlineimap") 'offlineimap 'mbsync))
         ...
         (cond
          ((executable-find "offlineimap")
           ...
           (defun fn/prodigy-offlineimap-state-alert (service state)
             ...
             (lexical-let ((service-name (plist-get service :name)))
               (when (string= service-name fn/imap-service-name)
                 (pcase state
         ...
         (setq fn/imap-service (prodigy-find-service fn/imap-service-name))

         (when fn/imap-service
           (prodigy-start-service fn/imap-service)
           (message "Auto-starting IMAP service."))))

     (add-hook 'after-init-hook #'fn/imap-service-setup)
   #+END_SRC
   ...
   #+BEGIN_SRC emacs-lisp :block-id 2f91f506-c8a6-85b4-769d-3fb460019887
     (use-package slack
       ...
       :bind (:map fn-standard-prefix-map
                   ...
                   ("s q" . slack-ws-close))
       :config
       ;; If the timeout is too low, it will fail.
                    ...
                    (lexical-let* ((config-name (fmwc/workgroup-config-name config))
                                   ...
                                   (workgroup-index
                                    (-find-index
                                     ...
                                     workgroups)))
                      (message "%s -> %s" index workgroup-index)
                      (cond
