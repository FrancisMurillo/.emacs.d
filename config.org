...
** Package Sources
   ...
   #+BEGIN_SRC emacs-lisp :block-id 8ec65bd3-dc31-0455-99d0-301267fdd059
     (defconst fn/package-archives
       (list
        (list "gnu" "http://elpa.gnu.org/packages/" 10)
        (list "org" "http://orgmode.org/elpa/" 0 )
        (list "melpa" "http://melpa.org/packages/" 20)
        (list "melpa-stable" "http://stable.melpa.org/packages/" 5))
       "List of my packages")
...
*** Grouped Services
    ...
    #+BEGIN_SRC emacs-lisp :block-id 1b36d539-6882-8a58-5206-512adfd9118e
      (fn/use-feature prodigy-set
        (prodigy)
        (require 'cl)

        (defvar fn/prodigy-service-sets nil
          "An alist of service sets.")

        (defun* fn/prodigy-define-service-set (&rest args)
          "Define a service set that will start and stop services."
          )

        (defun fn/prodigy-start-service-set (service-set)
          "Start a service set."
          )

        (defun fn/prodigy-stop-service-set (service-set)
          "Stop a service set."
          )

        (defun fn/prodigy-service-set-of-service (service)
          "Given a SERVICE, return the set it is in.")


        (advice-add 'prodigy-start-service :after 'ignore)
        (advice-add 'prodigy-stop-service :after 'ignore)
        (advice-add 'prodigy-set-status :after 'ignore)

        (fn/prodigy-define-service-set
         :name "w3m-privacy"
         :services '(("a" . ready)))
       )
    #+END_SRC
...
*** elpa-mirror
    ...
    #+BEGIN_SRC emacs-lisp :block-id d9ff5d65-9c4a-24ee-d9e8-81459f7851d3
      ...
      (defconst fn/mirror-elpa-id "elpa-mirror"
        "My mirror repo id.")
      ...
      (defconst fn/mirror-github-elpa-id "elpa-github-mirror"
        "Elpa github mirror id")

      (defconst fn/mirror-github-elpa-repo
        (format "https://raw.githubusercontent.com/%s/%s/master/"
                fn/user-github-name
                fn/mirror-elpa-repo)
        "Url of github elpa mirror")
      ...
      (use-package elpa-mirror
        ...
        ;; Local Mirror
        (add-to-list 'package-archives (cons fn/mirror-elpa-id fn/local-elpa-mirror-dir))
        (add-to-list 'package-archive-priorities (cons fn/mirror-elpa-id -1))

        ;; Github Mirror
        (add-to-list 'package-archives (cons fn/mirror-github-elpa-id fn/mirror-github-elpa-repo))
        (add-to-list 'package-archive-priorities (cons fn/mirror-github-elpa-id 0))
        ...
        (fn/use-feature paradox-elpa-mirror
          (elpa-mirror paradox)
...
*** prettify-symbols
    ...
    #+BEGIN_SRC emacs-lisp :block-id d6820a0e-ce89-788c-05e2-ceccda66bcb9
      (fn/use-feature prettify-symbols-mode
        nil)
    #+END_SRC
...
** org
   ...
   #+BEGIN_SRC emacs-lisp :block-id 4f5f3d2b-61ae-d33b-8316-4c20b3e9c8c0
     (use-package org
       ...
       :config
       ;; org files
       (defconst fn/org-dir (expand-file-name "~/Modules/nobody-library")
...
** ledger
   ...
   #+BEGIN_SRC emacs-lisp :block-id 4e3b93da-bda6-0881-7e81-3e23e833f1cc
     (defconst fn/ledger-file (expand-file-name "ledger.dat.gpg" "~/Modules/nobody-library")
       "My ledger file.")
     ...
     (fn/use-executables ledger-check
       ...
       (use-package ledger-mode
         ...
         (fn/use-feature ledger-capture
           (org ledger-mode)
           (when t ;; BUG: Emacs hangs with a GPG file for a capture
             (add-to-list
              'org-capture-templates
              (list "l" "Ledger" 'plain
                 (list 'file fn/ledger-file)
                 "%(org-read-date) * %?  \n\n"))))))
   #+END_SRC
...
** sendmail
   Sending mail is just as important as receiving it.

   #+BEGIN_SRC emacs-lisp :block-id 3a1e1a82-45f0-7f56-2597-b327ab24a413
     (use-package smtpmail
       :defer t
       :config
       (setq smtpmail-default-smtp-server "mail.ytandex.com"
          smtpmail-local-domain "fnmurillo.com"))
   #+END_SRC
...
*** Prettify
    ...
    #+BEGIN_SRC emacs-lisp :block-id cd91807f-6008-2cb5-5716-f6ee48077fc7
      (fn/use-function prettify-elisp
        prettify-symbols-mode
        (defun fn/prettify-elisp ()
          "Prettify elisp."
          ...
          (push '("defcustom" . ?τ) prettify-symbols-alist)
        ...
        (add-hook 'emacs-lisp-mode-hook #'fn/prettify-elisp))
    #+END_SRC
...
*** Prettify
    ...
    #+BEGIN_SRC emacs-lisp :block-id acb3e63d-ea49-d850-22c1-9e094a73b74a
      (fn/use-function prettify-js2
        prettify-symbols-mode
        (fn/use-feature prettify-js2
          (js2-mode)
          (defun fn/prettify-js2 ()
            "Prettify `js2-mode'."
            ...
            (prettify-symbols-mode t))
          (add-hook 'js2-mode-hook #'fn/prettify-js2)))
    #+END_SRC
*** JSON
    JSON support
...
*** Privacy Proxy
    ...
    #+BEGIN_SRC emacs-lisp :block-id 024bb8e1-c51e-c67b-b36b-77e031e445e5
      (fn/use-feature w3m-privacy-proxy
        (w3m)
        (fn/use-executables w3m-privoxy-polipo-tor
          ...
          (defun fn/w3m-privoxy-polipo-tor-update-conf ()
          ...
          (defun fn/w3m-remove-http-proxy-arguments ()
            ...
            (setq w3m-command-arguments ;; Remove added http_proxy arguments
               (cl-reduce  ;; I wish this could be easier
                (lambda (val xs)
                  (if (and (string= "-o" val)
                         (string-prefix-p "http_proxy=http://127.0.0.1:"
                                          (or (car xs) "")))
                      (cdr xs)
                    (cons val xs)))
          ...
          (defun fn/w3m-service-started-p ()
            "Check if the three main services are running."
            (and (prodigy-service-started-p fn/w3m-polipo-service)
               (prodigy-service-started-p fn/w3m-tor-service)
               (prodigy-service-started-p fn/w3m-privoxy-service)))


          (defun fn/w3m-service-start (&rest args)
            "Add http-proxy from `w3m-command-arguments'."
            (when (fn/w3m-service-started-p)
          ...
          (defun fn/w3m-service-init (&rest args)
            ...
            (unless (fn/w3m-service-started-p)
              (fn/w3m-privoxy-polipo-tor-update-conf)))
          ...
          (setq fn/w3m-polipo-service (prodigy-find-service "w3m-polipo")
             fn/w3m-tor-service (prodigy-find-service "w3m-tor")
             fn/w3m-privoxy-service (prodigy-find-service "w3m-privoxy"))


          (defun fn/w3m-start-privacy-services (&rest args)
            "Start services"
            (interactive)
            (prodigy-start-service fn/w3m-polipo-service)
            (prodigy-start-service fn/w3m-tor-service)
            (prodigy-start-service fn/w3m-privoxy-service))

          (defun fn/w3m-kill-privacy-services (&rest args)
            "Stop special services."
            (interactive)
            (prodigy-stop-service fn/w3m-polipo-service t)
            (prodigy-stop-service fn/w3m-tor-service t)
            (prodigy-stop-service fn/w3m-privoxy-service t))

          (add-hook 'kill-emacs-hook #'fn/w3m-kill-privacy-services)
          ...
          (when (yes-or-no-p "Start privacy services for w3m? ")
            (fn/w3m-start-privacy-services))))
    #+END_SRC
...
** erc
   ...
   #+BEGIN_SRC emacs-lisp :block-id 7cf5dadb-eb6b-a004-1566-eb12464e2e44
     (use-package erc
       ...
       (fn/use-feature erc-proxy-prodigy
         (prodigy)
         (fn/use-executables erc-tor-proxy
           (tor)

           (defcustom fn/erc-tor-port 18060
             "Tor proxy port to work use."
             :type 'number)

           (defcustom fn/erc-tor-cache-dir (expand-file-name "tor-erc-cache" fn/cache-dir)
             "Tor cache directory."
             :type 'directory)

           (defcustom fn/erc-tor-conf-file (expand-file-name "tor-erc-conf" fn/cache-dir)
             "My tor configuration location."
             :type 'file)


           (defun fn/erc-tor-update-conf ()
             "Create/update `fn/erc-tor-conf-file'' with new
     configurations."
             (interactive)
             (with-temp-file fn/erc-tor-conf-file
               (insert
                (string-join
                 (mapcar
                  (lambda (pair)
                    (pcase-let ((`(,key . ,value) pair))
                      (format
                       ...
                       (typecase value
                         ...
                         (numberp (number-to-string value))
                         (stringp value)))))
                  `(("SocksPort" . ,fn/erc-tor-port)
                    ("DataDirectory" . ,fn/erc-tor-cache-dir)))
                 "\n"))))


           (defun fn/erc-tor-service-init (&rest _args)
             "Setup erc-tor service"
             (fn/erc-tor-update-conf)
             (make-directory fn/erc-tor-cache-dir t)

             (setq socks-password ""
                socks-noproxy (list "localhost" "127.0.0.1" "irc.freenode.net"))

             (require 'socks)
             (require 'tls)

             (setq socks-server (list "erc-tor" "127.0.0.1" fn/erc-tor-port 5)
                url-gateway-method 'socks))

           (defun fn/erc-tor-service-on-start (&rest _args)
             "Erc Tor service on start"
             (setq erc-server-connect-function 'socks-open-network-stream))

           (defun fn/erc-tor-service-on-stop (&rest _args)
             "Erc Tor service on stop"
             (setq erc-server-connect-function 'erc-open-network-stream))

           (prodigy-define-tag
             :name 'erc
             :init #'fn/erc-tor-service-init
             :on-start #'fn/erc-tor-service-on-start
             :on-stop #'fn/erc-tor-service-on-stop)

           (fn/prodigy-define-service
            :name "erc-tor"
            :tags '(erc tor)
            :port fn/erc-tor-port
            :args (list "-f" fn/erc-tor-conf-file))

           (setq fn/erc-tor-service (prodigy-find-service "erc-tor"))

           (defun fn/erc-tor-kill-service ()
             "Kill `fn/erc-tor-service' service safely."
             (when (prodigy-service-started-p fn/erc-tor-service)
               (prodigy-stop-service fn/erc-tor-service t)))

           (when (yes-or-no-p "Start tor for erc? ")
             (prodigy-start-service fn/erc-tor-service))

           (add-hook 'kill-emacs-hook #'fn/erc-tor-kill-service))))
   #+END_SRC
   ...
   #+BEGIN_SRC emacs-lisp :block-id 0ebc0aed-f749-2d71-297a-692d5ab3b0a6
     (fn/use-executables emms-player-check
       ...
       (use-package emms
         ...
         (with-eval-after-load 'emms-player-vlc
         ...
         (with-eval-after-load 'emms-player-mpd
           (setq emms-player-mpd-server-name "localhost"
              emms-player-mpd-server-port 18600
...
** Initial View
   ...
   #+BEGIN_SRC emacs-lisp :block-id 3a40c500-5ad9-4d98-85de-75bc79d992f9
     (defun fn/setup-initial-view ()
       ...
       (when (fboundp 'fn/find-main-org-file)
         (fn/find-main-org-file))

       (select-frame-set-input-focus (selected-frame)))

     (add-hook 'after-init-hook #'fn/setup-initial-view t)
   #+END_SRC
...
** Session Configurations
   ...
   #+BEGIN_SRC emacs-lisp :block-id b8e02d73-638d-d7b5-2520-32fe159fdffb
     (fn/use-feature workgroup-configuration
       (workgroups2)
       (require 'dash)
     ...
     ICON-FAMILY, ICON-FUNCTION, ICON-NAME and ARGS."
       ...
       (fn/use-feature workspace-prodigy
         (prodigy)
